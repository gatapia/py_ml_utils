...
def __df_to_lines(df, 
  ...
  if out_file:
    with _get_write_file_stream(out_file) as outfile:    
      return impl(outfile)
  else: 
    return impl(None)
...
def _df_to_libffm(self, out_file_or_y=None, y=None):
  ...
  lines = []    
  outfile = None
  if out_file is not None: outfile = _get_write_file_stream(out_file)
...
def _get_write_file_stream(file):
...
_extend_df('ohe', _df_one_hot_encode)
_extend_df('toidxs', _df_to_indexes)
_extend_df('rm', _df_remove)
