...
class LibFMClassifier(BaseEstimator, ClassifierMixin):
  ...
  def fit(self, X, y):  
    self._fm = FM(self.num_iter, self.k0, self.k1, self.init_stdev, 
      self.validation_size, self.learning_rate_schedule, 
      self.initial_learning_rate, self.power_t, self.t0, self.task)  
    self._fm.fit(X, y)
    return self
