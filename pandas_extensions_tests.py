...
class T(unittest.TestCase):
  ...
  def test_engineer_rolling_sum_on_single_col(self):
    ...
    col = 'n_1(rolling_sum[3])'
    df.engineer(col)
    np.testing.assert_array_equal([np.nan, np.nan, 35, 40, 30, 29, 48], df[col])

  def test_engineer_rolling_mean_on_single_col(self):
    df = pd.DataFrame({'n_1': [10, 12, 13, 15, 2, 12, 34]})
    col = 'n_1(rolling_mean[3])'
    df.engineer(col)
    np.testing.assert_allclose([np.nan, np.nan, 11.66, 13.33, 10, 9.66, 16], df[col], rtol=1e-3)

  def test_engineer_rolling_median_on_single_col(self):
    df = pd.DataFrame({'n_1': [10, 12, 13, 15, 2, 12, 34]})
    col = 'n_1(rolling_median[3])'
    df.engineer(col)
    np.testing.assert_array_equal([np.nan, np.nan, 12, 13, 13, 12, 12], df[col])

  def test_engineer_rolling_min_on_single_col(self):
    df = pd.DataFrame({'n_1': [10, 12, 13, 15, 2, 12, 34]})
    col = 'n_1(rolling_min[3])'
    df.engineer(col)
    np.testing.assert_array_equal([np.nan, np.nan, 10, 12, 2, 2, 2], df[col])

  def test_engineer_rolling_max_on_single_col(self):
    df = pd.DataFrame({'n_1': [10, 12, 13, 15, 2, 12, 34]})
    col = 'n_1(rolling_max[3])'
    df.engineer(col)
    np.testing.assert_array_equal([np.nan, np.nan, 13, 15, 15, 15, 34], df[col])

  def test_engineer_rolling_std_on_single_col(self):
    df = pd.DataFrame({'n_1': [10, 12, 13, 15, 2, 12, 34]})
    col = 'n_1(rolling_std[3])'
    df.engineer(col)
    np.testing.assert_allclose([np.nan, np.nan, 1.528, 1.528, 7, 6.807, 16.371], df[col], rtol=1e-3)

  def test_engineer_rolling_var_on_single_col(self):
    df = pd.DataFrame({'n_1': [10, 12, 13, 15, 2, 12, 34]})
    col = 'n_1(rolling_var[3])'
    df.engineer(col)
