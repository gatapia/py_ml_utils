import unittest, sklearn, sklearn.linear_model, \
  sklearn.decomposition, datetime
...
class T(unittest.TestCase):
  ...
  def test_cats_to_count_with_binary_target(self):
    np.random.seed(0)
    df = pd.DataFrame({'c_1':['a', 'b', 'c'] * 5})
    y = pd.Series(np.random.rand(15))
    df.cats_to_counts(y)
    # print df
    # self.assertEqual(df.c_1, [1] * 15)
    # TODO

  def test_cats_to_count_ratios(self):
    pass
